...
- The two files were produced by **amalgamation** (concatenate pruned upstream files; inline
  the dolbeau `u*.h` / poly1305 `donna32/64.h` implementation-body headers; bracket the SIMD
  variants that reuse the same `static` names with `#define`/`#undef`; `#undef` each file's
  local macros at its section end). This was done by Claude Code as a one-off task, not a
  script — it's a reproducible agent task, so if re-syncing libsodium, just redo the
  amalgamation the same way (the pre-amalgamation per-file sources are in git history). See
  `sodium/NOTES.md`. (Not a "missing tool" — do not treat it as a gap to fill.)
...
- *Ergonomics are low-level by design.* You hand-write serialization, manually ref-count and
  ...
  fixed.
- *Build:* on CMake (was premake). The vendored libsodium is a pruned + amalgamated subset
  regenerated by an agent on demand, not by a checked-in script — that's intentional, not a
  gap.
